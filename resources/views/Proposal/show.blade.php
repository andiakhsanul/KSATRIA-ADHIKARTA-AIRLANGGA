...
@section('content')
    ...
    <div class="pb-4 mb-6">
        ...
        <div class="mt-2">
            ...
            @else
                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100">
                    {{ $proposal->status }}
                </span>
            @endif
        </div>
        ...
        @if (count($proposal->reviews) > 0)
            <div class="space-y-4">
                @foreach ($proposal->reviews as $review)
                    <div class="p-4 rounded-lg border border-zinc-700">
                        <div class="mb-3">
    ...
    <div>
        <div class="flex flex-row items-center justify-between">
            ...
            <button onclick="openModal('uploadRevisi')"
                class="px-4 py-2 border border-gray-700 transition-all rounded-md hover:bg-green-500">Upload Revisi</button>
        </div>
            ...
            <x-modal id="uploadRevisi" title="Upload Revisi">
                <div class="rounded-lg shadow">
                    ...
                    <form action="{{ route('revisi.store', $proposal->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="file_revisi" class="block font-semibold">Upload File Revisi:</label>
                            <input type="file" name="file_revisi" class="border p-2 w-full rounded" required>
                        </div>
                        <button type="submit"
